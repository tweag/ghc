...
mkCmdEnv tc_meths
  ...
  where
    mk_bind (std_name, expr)
      = do { rhs <- dsExpr expr
           ; id <- newSysLocalDs (Var.Usages zeroUA) (exprType rhs)
...
mkFstExpr a_ty b_ty = do
    a_var <- newSysLocalDs (Var.Mult Omega) a_ty
    b_var <- newSysLocalDs (Var.Mult Omega) b_ty
    pair_var <- newSysLocalDs (Var.Mult Omega) (mkCorePairTy a_ty b_ty)
    return (Lam pair_var
...
matchEnvStack env_ids stack_id body = do
    ...
    tup_var <- newSysLocalDs (Var.Mult Omega) (mkBigCoreVarTupTy env_ids)
    let match_env = coreCaseTuple uniqs tup_var env_ids body
    pair_id <- newSysLocalDs (Var.Mult Omega) (mkCorePairTy (idType tup_var) (idType stack_id))
...
matchEnv env_ids body = do
    uniqs <- newUniqueSupply
    tup_id <- newSysLocalDs (Var.Mult Omega) (mkBigCoreVarTupTy env_ids)
...
matchVarStack (param_id:param_ids) stack_id body = do
    ...
    pair_id <- newSysLocalDs (Var.Mult Omega) (mkCorePairTy (idType param_id) (idType tail_id))
...
dsCmd ids local_vars stack_ty res_ty
    ...
    stack_id   <- newSysLocalDs (Var.Mult Omega) stack_ty
...
dsCmd ids local_vars stack_ty res_ty (HsCmdApp _ cmd arg) env_ids = do
    ...
    stack_id <- newSysLocalDs (Var.Mult Omega) stack_ty
    arg_id <- newSysLocalDsNoLP (Var.Mult Omega) arg_ty
...
dsCmd ids local_vars stack_ty res_ty
    ...
    param_ids <- mapM (newSysLocalDsNoLP (Var.Mult Omega)) pat_tys
    stack_id' <- newSysLocalDs (Var.Mult Omega) stack_ty'
...
dsCmdDo ids local_vars res_ty [dL->L loc (LastStmt _ body _ _)] env_ids = do
    ...
    let env_ty = mkBigCoreVarTupTy env_ids
    env_var <- newSysLocalDs (Var.Mult Omega) env_ty
...
dsCmdStmt ids local_vars out_ids (BindStmt _ pat cmd _ _) env_ids = do
    ...
    env_id <- newSysLocalDs (Var.Mult Omega) env_ty2
    uniqs <- newUniqueSupply
    let
    ...
    pair_id   <- newSysLocalDs (Var.Mult Omega) after_c_ty
    let
...
dsCmdStmt ids local_vars out_ids
    ...
    uniqs <- newUniqueSupply
    env2_id <- newSysLocalDs (Var.Mult Omega) env2_ty
    let
...
dsRecCmd ids local_vars stmts later_ids later_rets rec_ids rec_rets = do
    ...
    rec_id <- newSysLocalDs (Var.Mult Omega) rec_ty
    let
