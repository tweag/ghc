...
normalise_type ty
  ...
  where
    ...
    go (FunTy at@(ArgType { at_type = ty1 }) ty2)
      = do { (co1, nty1) <- go ty1
           ; (co2, nty2) <- go ty2
           ; r <- getRole
           ; return (mkFunCo r co1 co2, FunTy (at { at_type = nty1 }) nty2) }
...
coreFlattenTy = go
  where
    ...
    go env (FunTy at@(ArgType { at_type = ty1 }) ty2)
      = let (env1, ty1') = go env  ty1
            (env2, ty2') = go env1 ty2 in
        (env2, FunTy (at { at_type = ty1' }) ty2')
...
allTyCoVarsInTy = go
  where
    ...
    go (FunTy (ArgType _ ty1) ty2) = (go ty1) `unionVarSet` (go ty2)
