...
tc_pat penv (ViewPat expr pat _) overall_pat_ty thing_inside
  = do  {
        ...
        ; expr_wrap2 <- tcSubTypeET (pe_orig penv) (weightedThing $ overall_pat_ty) inf_arg_ty
...
tc_pat _ (LitPat simple_lit) pat_ty thing_inside
  = do  { let lit_ty = hsLitType simple_lit
        ; co <- unifyPatType simple_lit lit_ty (weightedThing pat_ty)
...
tcPatSynPat penv (L con_span _) pat_syn pat_ty arg_pats thing_inside
  = do  { let (univ_tvs, req_theta, ex_tvs, prov_theta, arg_tys, ty) = patSynSig pat_syn
        ...
        ; wrap <- tcSubTypeET (pe_orig penv) (weightedThing pat_ty) ty'
