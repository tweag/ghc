...
flatten_one (FunTy at@(ArgType _ ty1) ty2)
  = do { (xi1,co1) <- flatten_one ty1
       ; (xi2,co2) <- flatten_one ty2
       ; role <- getRole
       ; return (FunTy (at { at_type = xi1 }) xi2
...
split_pi_tys' ty = split ty ty
  ...
  split _       (FunTy (ArgType af arg) res)
                                 = let (bs, ty, named) = split res res
                                   in  (Anon (ArgType af arg) : bs, ty, named)
...
ty_con_binders_ty_binders' = foldr go ([], False)
  where
    ...
    go (Bndr tv (AnonTCB af))   (bndrs, n)
      = (Anon (ArgType af (tyVarKind tv))   : bndrs, n)
    {-# INLINE go #-}
