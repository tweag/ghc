...
coreToTopStgRhs dflags scope_fv_info (bndr, rhs)
  = do { (new_rhs, rhs_fvs, _) <- coreToStgExpr rhs
       ...
       ; let stg_rhs   = mkTopStgRhs dflags rhs_fvs (mkSRT lv_info) bndr bndr_info new_rhs
...
mkTopStgRhs _ rhs_fvs srt _ binder_info (StgLam bndrs body)
...
mkTopStgRhs _ rhs_fvs srt bndr binder_info rhs
  = StgRhsClosure noCCS binder_info
                  (getFVs rhs_fvs)
                  (getUpdateFlag bndr)
                  srt
