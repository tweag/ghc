...
specCase env scrut' case_bndr [(con, args, rhs)]
  ...
  where
    ...
    clone_me bndr = do { uniq <- getUniqueM
                       ; return (mkUserLocalOrCoVar occ uniq mult ty loc) }
       where
         name = idName bndr
         mult = idMult bndr
         ty   = idType bndr
...
newDictBndr env b = do { uniq <- getUniqueM
                       ...
                       ; return (mkUserLocalOrCoVar (nameOccName n) uniq (Regular Omega) ty' (getSrcSpan n)) }
...
newSpecIdSM old_id new_ty join_arity_maybe
  = do  { uniq <- getUniqueM
        ; let name    = idName old_id
              ...
              new_id  = mkUserLocalOrCoVar new_occ uniq (Regular Omega) new_ty (getSrcSpan name)
