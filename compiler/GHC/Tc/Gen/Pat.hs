...
tcInferPat ctxt pat thing_inside
  = tcInfer $ \ exp_ty ->
    tc_lpat pat (unrestricted exp_ty) penv thing_inside
 where
    penv = PE { pe_lazy = False, pe_ctxt = LamPat ctxt, pe_orig = PatOrigin }
...
tcCheckPat_O ctxt orig pat (Scaled pat_mult pat_ty) thing_inside
  = tc_lpat pat (Scaled pat_mult (mkCheckExpType pat_ty)) penv thing_inside
  where
...
tc_pat penv (ViewPat _ expr pat) overall_pat_ty thing_inside
  = do  { mult_wrap <- checkManyPattern overall_pat_ty
        ...
        ; expr_wrap2 <- tc_sub_type penv (scaledThing overall_pat_ty) inf_arg_ty
