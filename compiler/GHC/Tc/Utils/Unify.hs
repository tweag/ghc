...
matchActualFunTysPart
  ...
  -> [Scaled TcSigmaType]  -- Types of values args to which function has
...
-- in elsewhere).
  ...
  where
    ...
    go :: Arity
       -> [Scaled TcSigmaType] -- Types of value args to which the function has
    ...
    go n so_far (FunTy { ft_af = af, ft_mult = w, ft_arg = arg_ty, ft_res = res_ty })
      = ASSERT( af == VisArg )
        do { (wrap_res, tys, ty_r) <- go (n-1) (Scaled w arg_ty : so_far) res_ty
    ...
    ------------
    mk_ctxt :: [Scaled TcType] -> TcType -> TidyEnv -> TcM (TidyEnv, MsgDoc)
    mk_ctxt arg_tys res_ty env
