...
rnHsTyKi isType doc (HsKindSig ty k)
  = ASSERT ( isType ) 
    do { kind_sigs_ok <- xoptM Opt_KindSignatures
       ; unless kind_sigs_ok (addErr (badSigErr False doc ty))
...
checkHsBndrFlags is_type doc ty tv_bndrs
  ...
  = do { sig_ok <- xoptM Opt_ScopedTypeVariables
       ; unless sig_ok (addErr (badSigErr True doc ty)) }
  | otherwise   -- Kind
  = do { sig_ok <- xoptM Opt_KindSignatures
       ; unless sig_ok (addErr (badSigErr False doc ty))
...
badSigErr :: Bool -> HsDocContext -> LHsKind RdrName -> SDoc
badSigErr is_type doc ty
